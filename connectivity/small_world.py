...
# ---------------------------------------------------------
# Watts-Strogatz small world parameters for the entire network
N_ws = num_neurons      # number of nodes (500)
k_ws = int(eps_ee * num_neurons) # each node is connected to k_ws nearest neighbors
if k_ws % 2 != 0:
    k_ws += 1           # Watts-Strogatz degree parameter k must be even
p_ws = 0.1              # rewiring probability

graph = nx.watts_strogatz_graph(N_ws, k_ws, p_ws)

# Categorize directed edges for E->E, E->I, I->E, I->I
src_ee, tar_ee = [], []
src_ei, tar_ei = [], []
src_ie, tar_ie = [], []
src_ii, tar_ii = [], []
...
for node_u, node_v in graph.edges():
    if node_u != node_v:
        # Undirected edge (node_u, node_v) becomes directed edges node_u -> node_v and node_v -> node_u
        for edge_u, edge_v in [(node_u, node_v), (node_v, node_u)]:
            if edge_u < ne and edge_v < ne:
                src_ee.append(edge_u)
                tar_ee.append(edge_v)
            elif edge_u < ne and edge_v >= ne:
                src_ei.append(edge_u)
                tar_ei.append(edge_v - ne)
            elif edge_u >= ne and edge_v < ne:
                src_ie.append(edge_u - ne)
                tar_ie.append(edge_v)
            elif edge_u >= ne and edge_v >= ne:
                src_ii.append(edge_u - ne)
                tar_ii.append(edge_v - ne)

# Convert to numpy arrays
src_ee, tar_ee = np.array(src_ee), np.array(tar_ee)
src_ei, tar_ei = np.array(src_ei), np.array(tar_ei)
src_ie, tar_ie = np.array(src_ie), np.array(tar_ie)
src_ii, tar_ii = np.array(src_ii), np.array(tar_ii)

S_EE = Synapses(exc, exc, model=exc_plastic_model, on_pre=exc_on_pre, method='euler')
S_EE.connect(i=src_ee, j=tar_ee)
S_EE.w = J_exc
S_EE.x_bar = 0

S_EI = Synapses(exc, inh, model=exc_plastic_model, on_pre=exc_on_pre, method='euler')
S_EI.connect(i=src_ei, j=tar_ei)
S_EI.w = J_exc
S_EI.x_bar = 0
